# -*- coding: utf-8 -*-
"""
Created on Thu Aug  6 00:21:15 2023
@author: Qunlun Shen
...
def align_adata_vars(adata1: sc.AnnData,  # better be raw data
    ...
    if not homologous:
        ...
        var_common = np.intersect1d(vars_all1, vars_all2)
        adata1 = adata1[:, var_common].copy()
        adata2 = adata2[:, var_common].copy()
        return adata1, adata2
    else:
        vars_all1, vars_all2 = list(map(all_vars_of_adata, [adata1, adata2]))
        print('len(vars_all1), len(vars_all2)', len(vars_all1), len(vars_all2))
        submaps_1v1 = subset_matches(df_varmap_1v1, vars_all1, vars_all2, union=False)
        print('submaps_1v1', submaps_1v1)
...
def quick_preprocess(
    ...
    if do_pca and nneigh is not None:
        if not spatial:
            ...
            if (spatial_type == 'slide_seqV2') or (spatial_type == 'slide_seq'):
                cal_slide_net(_adata)
            if spatial_type == '10X':
                cal_ST_net(_adata)
...
def cal_slide_net(adata, avg_neibor=10):
    locations = pd.concat([adata.obs['array_row'], adata.obs['array_col']], axis=1)
    locations.index = adata.obs.index
    n_spots = locations.shape[0]
    nbrs = NearestNeighbors(n_neighbors=avg_neibor+1, algorithm='ball_tree', metric='euclidean', n_jobs=-1).fit(locations)
    distances, indices = nbrs.kneighbors(locations)
    avg_distance = np.mean(distances[:,avg_neibor])
    print('avg_neibor', avg_neibor)
    print('distances', distances[:,avg_neibor])
    print('avg distances', np.mean(distances[:,avg_neibor]))
    print('the nearnest neighbors distances are:', np.mean(distances[:,1]))
    ...
    for i in range(n_spots):
        for j in range(1, avg_neibor+1):
            if distances[i,j] <= avg_distance:
                counts += 1
                data.append(distances[i,j])
                _indices.append(indices[i,j])
        indptr.append(counts)
    data = np.array(data)
    indptr = np.array(indptr)
    _indices = np.array(_indices)
    spa_net = sparse.csr_matrix((data, _indices, indptr), shape=(n_spots, n_spots))
    adata.obsp['spa_nets'] = spa_net

def cal_ST_net(adata: sc.AnnData):
    ...
    locations = pd.concat([adata.obs['array_row'], adata.obs['array_col']], axis=1)
    n_spots = locations.shape[0]
    spa_net = pd.DataFrame(np.zeros((n_spots, n_spots)), index=list(locations.index), columns=list(locations.index))
    for i in range(n_spots):
        spot_name = locations.index[i]
        x, y = locations.iloc[i, 0:2]
        for (x_shrift, y_shrfit) in ((0, -2), (0, 2), (-1, 1), (1, 1), (-1, -1), (1, -1)):
            loc_place = locations[(locations['array_row'] == x+x_shrift)  & (locations['array_col'] == y+y_shrfit)]
            if len(loc_place):
                neigh_spot_name = loc_place.index[0]
                spa_net.loc[spot_name, neigh_spot_name] = 1
    spa_net = sparse.csr_matrix(np.array(spa_net))
    adata.obsp['spa_nets'] = spa_net
...
def top_markers_from_adata(adata: sc.AnnData,
    ...
    #df1 = get_marker_info_table(adata, groups=None,
                          #key='rank_genes_groups')
    #df1.to_csv('DEG.csv')
    #df1_fdr = df1.loc[df1['pvals_adj'] < 0.01, :]
    #genes_fdr = df1_fdr.loc[:, 'names']
    #genes_unique = set(genes_fdr)
    #return list(genes_unique)
